import { none } from "./transformers.js";

// chinese/japanese/russian chars https://stackoverflow.com/a/43419070
const NON_ENGLISH_CHARS =
  "\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f\u0400-\u04ff";
const RUSSIAN_CAST_REGEX = new RegExp(
  "\\([^)]*[\u0400-\u04ff][^)]*\\)$|(?<=\\/.*)\\(.*\\)$",
);
const ALT_TITLES_REGEX = new RegExp(
  `[^/|(]*[${NON_ENGLISH_CHARS}][^/|]*[/|]|[/|][^/|(]*[${NON_ENGLISH_CHARS}][^/|]*`,
  "g",
);
const NOT_ONLY_NON_ENGLISH_REGEX = new RegExp(
...
const NOT_ALLOWED_SYMBOLS_AT_START_AND_END = new RegExp(
  `^[^\\w${NON_ENGLISH_CHARS}#[【★]+|[ \\-:/\\\\[|{(#$&^]+$`,
  "g",
);
const REMAINING_NOT_ALLOWED_SYMBOLS_AT_START_AND_END = new RegExp(
  `^[^\\w${NON_ENGLISH_CHARS}#]+|]$`,
  "g",
);
...
function createHandlerFromRegExp(name, regExp, transformer, options) {
  function handler({ title, result, matched }) {
    ...
    if (rawMatch) {
      ...
      const isBeforeTitle =
        beforeTitleMatch && beforeTitleMatch[1].includes(rawMatch);
  ...
  addHandler(handlerName, handler, transformer, options) {
    if (typeof handler === "undefined" && typeof handlerName === "function") {
      ...
      options = extendOptions(
        typeof transformer === "object" ? transformer : options,
      );
      handler = createHandlerFromRegExp(
        handlerName,
        handler,
        transformer,
        options,
  ...
  parse(title) {
    ...
    for (const handler of this.handlers) {
      ...
      if (matchResult && matchResult.remove) {
        title =
          title.slice(0, matchResult.matchIndex) +
          title.slice(matchResult.matchIndex + matchResult.rawMatch.length);
      }
      if (
        matchResult &&
        !matchResult.skipFromTitle &&
        matchResult.matchIndex &&
        matchResult.matchIndex < endOfTitle
      ) {
