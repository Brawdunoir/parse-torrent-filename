...
describe("Parsing source", () => {
  it("should detect the Bluray source correctly", () => {
    ...
    assert.deepStrictEqual(parse(releaseName).source, "BluRay");
  });

  it("should detect the HDTV source correctly", () => {
    const releaseName =
      "doctor_who_2005.8x12.death_in_heaven.720p_hdtv_x264-fov";
    assert.deepStrictEqual(parse(releaseName).source, "HDTV");
  });
  ...
  it("should detect the MicroHD source correctly", () => {
    ...
    assert.deepStrictEqual(parse(releaseName).source, "HDRip");
  });

  it("should detect the UHDrip source correctly", () => {
    const releaseName =
      "Bohemian Rhapsody 2018.2160p.UHDrip.x265.HDR.DD+.5.1-DTOne";
    assert.deepStrictEqual(parse(releaseName).source, "UHDRip");
  });
  ...
  it("Solo: A Star Wars Story (2018) English 720p TC x264 900MBTEAM TR", () => {
    ...
    assert.deepStrictEqual(parse(releaseName).source, "TeleCine");
  });
  ...
  it("You're Next (2013) cam XVID", () => {
    ...
    assert.deepStrictEqual(parse(releaseName).source, "CAM");
  });
  ...
  it("Vampire in Vegas (2009) NL Subs DVDR DivXNL-Team", () => {
    ...
    assert.deepStrictEqual(parse(releaseName).source, "DVD");
  });
  ...
  it("should parse BluRay Rip source", () => {
    ...
    assert.deepStrictEqual(parse(releaseName).source, "BRRip");
  });

  it("should parse BluRay remux together source", () => {
    const releaseName =
      "Joker.2019.2160p.BluRay.REMUX.HEVC.DTS-HD.MA.TrueHD.7.1.Atmos-FGT";
    assert.deepStrictEqual(parse(releaseName).source, "BluRay REMUX");
  });
